...
test_that("Length output", {
  expect_equal(length(labelPrediction(umap_obj, new_clusters)), 4)
})

test_that("Length output - Update", {
  expect_equal(length(labelPrediction(umap_obj, new_clusters, update_reference = TRUE, reference = annotated_M)), 5)
})
