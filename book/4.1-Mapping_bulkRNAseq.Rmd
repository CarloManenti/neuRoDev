...
## Quickstart
These are the main functions to map a bulk RNA-sequencing dataset on the corticogenesis resource. This implies that the data are already normalized. 
...
data <- readRDS('~/Downloads/my_data') #choose your data object
...
```
Below we report a detailed description and examples on how to perform mapping.
...
This example dataset from Gordon et al. (2021) (doi: 10.1038/s41593-021-00802-y) can be downloaded from: ...
...
To check the expression of preferentially expressed genes defined within our resource in the mapped samples. The objects are available at: ...
...
We show here a function to do this, which is not included in the package (to keep only the main functions). The function `plotTrends` requires the following inputs:
...
This is consistent with the expression levels of GO processes manually curated in our compendium. These are available at: ... and represent essential pathways in the processes of corticogenesis, neurogenesis, and gliogenesis.
...
To compare the preferential expression of GO in our resource, it is possible to download the preferential expression of Gene Ontology Biological Processes (BP), Molecular Functions (MF), and Cellular Components (CC) here. Each object is a list containing preferential expression scores in one of the three reference networks in the three ontologies (BP, MF, CC). Each element of each list contains the activity (`activity`) derived from Gene Set Variation Analysis (one value per gene set in each cluster) and the preferential expression scores (`preferential`; one value per gene set in each subclass).
