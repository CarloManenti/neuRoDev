...
#'
#' Function to automatically download the neurogenesis reference network along
...
#' }
#'
#' @source Obtained by processing the excitatory neuron lineage of the
#' single-cell data compendium.
#' @export
neurogenesis.sce <- function(directory=NULL) {
  figshare_url <- "https://api.figshare.com/v2/file/download/60381032"
  ...
  destfile <- file.path(directory, "neurogenesis.sce.rda")

  if (!file.exists(destfile)) {
    message("Downloading the neurogenesis.sce object (~149MB) to:\n", destfile)
