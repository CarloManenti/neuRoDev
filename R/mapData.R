...
mapData <- function(annotated_reference,
  ...
  mean_sig <- mean(new_signatures > 1)
  if(mean_sig < 0.05) {
    warning(paste0('The signatures given have less then 5% (', round(mean_sig*100, digits = 1), '%)
  ...
  if(mean_sig > 0.20) {
    warning(paste0('The signatures given have more then 20% (', round(mean_sig*100, digits = 1), '%)
    of the genes with significant variability in the samples (fold-change > 2).
