...
mapGenes <- function(umap_obj,
  ...
  for(gene in gene_set) {
    ...
    if(sum(weights) == 0) {
      gene_set <- gene_set[which(!gene_set %in% gene)]
      next()
    }
  ...
  min_distances <- c()
  idxs_dist <- c()
  for(gene in seq(1, dim(gene_points)[1])) {
    gp <- gene_points[gene,]
  ...
  for(g in unique(color_attr)) {
    ...
    if(length(closest_genes) == 0) {
      ...
      if(is.vector(sub_n_ref)) {
        sub_n_ref <- t(as.matrix(sub_n_ref))
      }
      sub_x_coord <- c()
      sub_y_coord <- c()
      sub_min_distances <- c()
      ...
      x_coord[closest_genes] <- sub_n_ref[sub_idxs_dist[match(closest_genes, gene_set)], 1]
      y_coord[closest_genes] <- sub_n_ref[sub_idxs_dist[match(closest_genes, gene_set)], 2]
