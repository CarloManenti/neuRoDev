...
scoreMapping <- function(net,
  ...
  max_mean_cors <- t(get_column_group_average(t(max_cor), interaction(label_attr,
                                                                      net$Stages, sep = "and")))
  ...
  by_subclass_precision <- tapply(precision, unlist(lapply(strsplit(names(precision),
                                                                    "and", fixed = TRUE), function(i) {i[1]})), mean)
